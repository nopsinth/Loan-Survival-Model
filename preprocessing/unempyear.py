...
with open('unemployyear4.csv', 'r') as f:
    data = csv.reader(f)
    for i, line in enumerate(data):
        if i == 0: continue
        for state in range (0,51):
            if state == 0: continue
            unemployment[i*100 + state] = line[state] #keep data in dictionary dummy:rate ; dummy = 100*time+state

with open('test_outsample1.csv', 'r') as g:
    ...
    for i, line in enumerate(dataTwo):
        ...
        mydata.append(line + [unemployment[dummy]])

with open('test_outsample2.csv', 'w') as h:
     writer = csv.writer(h)
